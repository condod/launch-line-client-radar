...
The voice system has two live workflows:
...
5. Configure the agent, transfer number, calling window, disclosure, and attempt limit.
...
2. Create a test prospect using another number you own.
3. Record test written-consent evidence, enable AI outbound, and approve one call.
4. Say "do not call me again" and verify the server call history shows `do-not-call`.
5. Confirm that a second AI attempt inside 24 hours is rejected.
